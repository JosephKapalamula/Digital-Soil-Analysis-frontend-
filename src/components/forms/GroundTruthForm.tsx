...
export default function GroundTruthForm({
  lat,
  lon,
  onClose,
  onSuccess,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      await soilService.collectGroundTruth({
        location: `${lat},${lon}`,
        latitude: lat,
        longitude: lon,
        ...formData,
      ...
      >
        <div className="bg-emerald-50 p-4 rounded-lg border border-emerald-100 flex gap-3">
          ...
          <p className="text-xs text-emerald-800">
            You are recording a physical lab result for{" "}
            <strong>
        ...
        <div className="space-y-4">
          ...
          <div>
            ...
            <select
              ...
              onChange={(e) =>
                setFormData({ ...formData, recommended_crop: e.target.value })
              }
            >
              <option>Maize</option>
        ...
        >
          {loading ? (
            ...
            <>
              <Save size={18} /> Save to Ground Truth
            </>
          )}
        </button>
      </form>
